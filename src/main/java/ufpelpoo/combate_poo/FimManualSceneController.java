...
public class FimManualSceneController extends JogoManualController {
    ...
    public void initialize(){
        Label label;
        if(JogoManualController.getGanhouAzul()==true){
    ...
    void onReiniciarClick(ActionEvent event) throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(MainApplication.class.getResource("ManualScene.fxml"));
        Stage stage = new Stage();
        stage.setTitle("COMBATE");
        stage.setScene(new Scene(fxmlLoader.load(), 960, 540));
        stage.setResizable(false);
